...
import (
	...
	"errors"
	"fmt"
	"github.com/KercyLAN/secret-dimension-core/extra"
...
type FileUploadReceiver struct {
	*partProcessor                                       // 分块上传处理器
	...
	buildID					 func() string				 // ID生成函数
	node 					 Node						 // 该文件处理器所在节点
}
...
// 设置所在节点
func (slf *FileUploadReceiver) SetNode(node Node) {
	slf.node = node
}

// 设置ID生成器，默认为UUID
func (slf *FileUploadReceiver) SetIDBuilder(builder func() string) {
	slf.buildID = builder
}
...
func (slf *FileUploadReceiver) DisposePartMerge(hash string, fileName string) (*FileMeta, error) {
	...
	fileMeta = newFileMeta(slf.buildID(), fileName, slf.getPartSize(hash), slf.node)
	...
	var eachErr error
	extra.EachMapSort(slf.task[hash], func(partIndex int, data []byte) {
		if len(data) == 0 {
			eachErr = read.ReadBlockHook(slf.taskTemp[hash][partIndex], int(slf.saveFileBufferSize), func(data []byte) {
...
func (slf *FileUploadReceiver) DisposePart(request *http.Request) (string, error) {
	...
	if fileHeader.Size > slf.partUploadBufferSize {
		filePath := fmt.Sprintf("%v/%v/%v.part.temp", slf.storageDir, "temp", slf.buildID())
